...
function(qt_cmake_configure)
    ...
    # Overlay fix: the DYNAMIC host qtbase build (x64-windows, built by vcpkg to
    # provide moc/uic/rcc for the static target) runs uic/rcc/qtpaths as in-tree
    # "try_run" tools near the end of its build. Those load Qt6Core*.dll plus
    # third-party DLLs (icu/pcre2/zstd/double-conversion) that live in the host
    # installed dir's bin/, which is NOT on PATH — so the try_run aborts with
    # 0xc0000135 (STATUS_DLL_NOT_FOUND). Prepend the host bin dirs (and the
    # build-tree bin where Qt6Cored.dll is produced) so the try_run can load
    # them. Only the dynamic link emits DLLs, so gate on it; the static target
    # build is untouched.
    if(VCPKG_LIBRARY_LINKAGE STREQUAL "dynamic")
        vcpkg_add_to_path(PREPEND
            "${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-dbg/bin"
            "${CURRENT_HOST_INSTALLED_DIR}/debug/bin"
