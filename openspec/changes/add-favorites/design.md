...
A bookmark control is added to the shared `Control` enum and placed immediately to the right of the zoom-in button in both the Win32 `layout()`/`defs[]` tables and the Qt `addButton` sequence. The marked/unmarked state is carried by the **icon**, not a pressed-button background: `ToolbarPresenter` calls `setIcon(Control::ToggleFavorite, …)` with Material `bookmark` (U+E8E7) when the current page is unmarked and `bookmark_star` (U+F454) when marked — the same state-driven icon swap `MatchCase` already uses. The button is therefore not checkable on either backend. `ToolbarPresenter` re-syncs the icon on every page change so keyboard, mouse, and toolbar never diverge.

The embedded `assets/MaterialSymbolsOutlined.ttf` was re-carved to add those two codepoints (26 glyphs, ~4.8 KB) from the Material Symbols Outlined variable font at FILL 0/GRAD 0/opsz 24/wght 400; the previously carved 24 glyph outlines are byte-identical (the subset now also fixes the earlier `Icon::Favorites` codepoint, which was the Material *Icons* `0xe866` and absent from the subset, so that button had always fallen back to the vector glyph).
...
- **Windows (`src/viewer_win32.cpp`, `src/toolbar_win32.cpp`, `src/sidebar_win32.cpp`, `src/pluginconfig.cpp`):** `Ctrl+B` in the hotkey switch; bookmark button in the `layout()`/`defs[]` row to the right of zoom-in, state shown by the `bookmark`/`bookmark_star` glyph (not a pressed background); sidebar `clearEntries`/build appends the flat Favorites section via `addEntry` and captures/restores `TVIS_EXPANDED` state; module-dir resolution reuses the existing `GetModuleHandleW` code.
