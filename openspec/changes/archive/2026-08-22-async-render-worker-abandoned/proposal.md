> **STATUS: ABANDONED / NOT SHIPPED.** This change was prototyped and fully
> implemented, then reverted. The background render worker and its thread-safety
> plumbing (MuPDF clone-context + locks, DjVu mutex, FIFO/drop-window ordering,
> viewer marshaling, placeholders) did not justify the latency gain for a single
> lister panel with a per-page LRU cache already in place. A paged-mode FIFO bug
> surfaced in real use and upstream sync-on-demand already keeps first-look
> stalls bounded to one or two pages. The codebase intentionally remains
> synchronous (`renderPageCached` renders on first touch on the UI thread).
> Nothing from this change is synced to `openspec/specs/`. Recorded for future
> reference only.
