...
- [x] 5.1 Archive / sync: once verified, sync the `viewer-scrolling` delta into `openspec/specs/` and propose the `async-render-worker` change to remove the residual synchronous render stall (already planned)