...
The info panel SHALL display the current fit mode. The status reads `Fit: Page` when the viewer is in fit-to-page mode, `Fit: Width` when in fit-to-width mode, and `Zoom: <pct>%` when in manual zoom (where `<pct>` is the current zoom percentage rounded to the nearest integer, e.g. `Zoom: 100%` at 1.0x, `Zoom: 150%` at 1.5x).
