# Viewer Display Modes Specification

## Purpose

Defines the viewer's two display modes for paging through a document - paged (one view unit at a time, mouse wheel jumps strictly unit to unit) and continuous (view units stacked vertically, mouse wheel scrolls smoothly across unit boundaries) - and how the user switches between them. A view unit is one page in single-page presentation, two adjacent pages in double page presentation, or a cover page alone followed by two-page pairs in double page with cover presentation (see `viewer-display-modes/two-page-view`). Applies to both the Win32 viewer on Windows and the Qt viewer on Linux.
